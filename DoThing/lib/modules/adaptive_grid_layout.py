...
class Adaptive_GridLayout(GridLayout):
    ...
    def __init__(self, grow_cols = False, grow_rows = False, **kwargs):
        self.c_debug = 0
        if self.c_debug>0: print("Adaptive_GridLayout.__init__():", self)
        ...
        if self.c_debug > 1: print("Adaptive_GridLayout.__init__() finished")

    def _yield_tallest_of_each_row(self):
        """ Yields tallest child of each row within gridlayout. """
        if self.c_debug>0: print("\t\t\t\tAdaptive_GridLayout._yield_tallest_of_each_row():", self)
        ...
        for i, c in enumerate(list(reversed(self.children))):
            ...
            if self.cols is None or self.cols is 0:
                if self.c_debug > 1: print("\t\t\t\tcurrent_tallest:", current_tallest, current_tallest.height)
    ...
    def _calc_child_padding_y(self, child):
        ...
        if self.c_debug>0: print("\t\t\t\tAdaptive_GridLayout._calc_child_padding_y():", self, child)
        ...
        if len_child_padding is 1:
            ...
            padding = 0
        if self.c_debug>1: print("\t\t\t\tAdaptive_GridLayout._calc_child_padding_y() padding after:", padding)
        return padding
    ...
    def _calc_min_height(self):
        ...
        if self.c_debug>0: print("\t\t\tAdaptive_GridLayout._calc_min_height():", self)
        min_height = 0
        for c in self._yield_tallest_of_each_row():
            min_height += c.height
        if self.c_debug>1: print("\t\t\tAdaptive_GridLayout._calc_min_height() min_height after:", min_height)
        return min_height
    ...
    def _calc_rows_minimum(self):
        ...
        if self.c_debug>0: print("\t\t\tAdaptive_GridLayout._calc_rows_minimum():", self)
        rows_minimum = OrderedDict()
        for i, c in enumerate(self._yield_tallest_of_each_row()):
            rows_minimum.update({i: c.height})
        if self.c_debug>1: print("\t\t\tAdaptive_GridLayout._calc_rows_minimum() row_minimum after:", rows_minimum)
        return rows_minimum

    def _refresh_height(self):
        """ Resets 'self.height' using value returned by '_calc_min_height' method. """
        if self.c_debug>0: print("\t\tAdaptive_GridLayout._refresh_height():", self)
        if self.c_debug>0: print("\t\tAdaptive_GridLayout._refresh_height() self.height before:", self.height)
        if self.c_debug>0: print("\t\tAdaptive_GridLayout._refresh_height() self.minimum_height before:", self.minimum_height)
        self.height = self._calc_min_height()
        #self._update_minimum_size() #this doesn't work, but it should do the same thing
        if self.c_debug>1: print("\t\tAdaptive_GridLayout._refresh_height() self.height after:", self.height)

    def _refresh_rows_minimum(self):
        """ Resets 'self.rows_minimum' using value returned by '_calc_rows_minimum' method. """
        if self.c_debug>0: print("\t\tAdaptive_GridLayout._refresh_rows_minimum():", self)
        if self.c_debug>0: print("\t\tAdaptive_GridLayout._refresh_rows_minimum() self.rows_minimum before:", self.rows_minimum)
        self.rows_minimum = self._calc_rows_minimum()
        if self.c_debug>1: print("\t\tAdaptive_GridLayout._refresh_rows_minimum() self.rows_minimum after:", self.rows_minimum)
    ...
    def _refresh_y_dimension(self):
        ...
        if self.c_debug>0: print("\tAdaptive_GridLayout._refresh_y_dimension():", self)
        ...
        if self.c_debug > 1: print("\tAdaptive_GridLayout._refresh_y_dimension() finished")

    def on_children(self, instance, value):
        """ If 'grow_cols' or 'grow_rows' is True this will grow layout that way if needed instead of erroring out. """
        if self.c_debug>0: print("Adaptive_GridLayout.on_children():", self, instance, value)
    ...
    def on_parent(self, instance, value):
        ...
        if self.c_debug>0: print("Adaptive_GridLayout.on_parent():", self, instance, value)
